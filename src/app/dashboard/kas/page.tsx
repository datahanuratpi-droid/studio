      ...
      <div className="grid grid-cols-1 gap-4 px-1">
        ...
        <Card className="bg-primary text-white border-none shadow-xl relative overflow-hidden rounded-3xl h-32 flex flex-col justify-center">
          <CardContent className="p-6 relative z-10">
            ...
            <p className="text-2xl md:text-3xl font-black font-headline tracking-tighter">Rp {totalSaldo.toLocaleString('id-ID')}</p>
          </CardContent>
                ...
                <TableBody>
                  {isLoading ? (
                    ...
                    <TableRow key={t.id} className="hover:bg-muted/5 transition-colors border-border/30">
                      <TableCell className="text-[10px] font-medium text-muted-foreground whitespace-nowrap pl-6">
                        {new Date(t.transactionDate).toLocaleDateString('id-ID', { day: '2-digit', month: 'short' })}
                      </TableCell>
                      <TableCell>
                        <div className="flex flex-col">
                          <span className="font-bold text-primary text-[11px] uppercase tracking-tight line-clamp-1">{t.description}</span>
                          <span className={cn("text-[8px] font-black uppercase tracking-widest", 
                            t.type === 'Receipt' ? "text-green-600" : "text-blue-600"
                          )}>
                            {t.type === 'Receipt' ? 'Masuk' : 'Kantor'}
                          </span>
                        </div>
                      </TableCell>
                      <TableCell className="whitespace-nowrap">
                        <span className={cn("font-black text-[11px]", t.type === 'Receipt' ? "text-green-600" : "text-red-600")}>
                          {t.type === 'Receipt' ? '+' : '-'} {t.amount.toLocaleString('id-ID')}
                        </span>
                      </TableCell>
                      <TableCell className="text-right pr-6">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-40 rounded-2xl p-2">
                            <DropdownMenuItem onClick={() => setViewingTransaction(t)} className="cursor-pointer rounded-xl text-xs font-bold"><Activity className="mr-2 h-3.5 w-3.5" /> Detail</DropdownMenuItem>
                            {isAdmin && !t.isClosed && (
                              <DropdownMenuItem onClick={() => handleDelete(t.id)} className="text-red-600 cursor-pointer rounded-xl text-xs font-bold"><TrendingDown className="mr-2 h-3.5 w-3.5" /> Hapus</DropdownMenuItem>
                            )}
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </TableCell>
                  ...
                  {sortedTransactions.length === 0 && !isLoading && (
                    <TableRow>
                      <TableCell colSpan={4} className="text-center py-20 text-muted-foreground text-[10px] uppercase tracking-widest font-bold opacity-30">
                        Tidak ada data.
                      </TableCell>
                    </TableRow>
                  )}
        ...
        <DialogContent className="w-[95vw] sm:max-w-[480px] rounded-3xl p-0 overflow-hidden border-none shadow-2xl">
          <form onSubmit={handleSubmit} className="flex flex-col">
            <DialogHeader className="p-6 bg-primary text-white">
              ...
              <DialogDescription className="text-white/80 text-[10px] font-medium">Input data operasional kantor Hanura.</DialogDescription>
            </DialogHeader>
            <div className="p-6 space-y-6">
              <div className="space-y-2">
                <Label className="font-black text-[9px] uppercase text-muted-foreground tracking-widest">Jenis Transaksi</Label>
                <Select onValueChange={setSelectedType} value={selectedType}>
                  <SelectTrigger className="h-12 rounded-2xl text-xs border-muted-foreground/20"><SelectValue /></SelectTrigger>
              ...
              <div className="space-y-2">
                <Label className="font-black text-[9px] uppercase text-muted-foreground tracking-widest">Jumlah (Rp)</Label>
              ...
              <div className="space-y-2">
                <Label className="font-black text-[9px] uppercase text-muted-foreground tracking-widest">Keterangan</Label>
                <Input name="description" placeholder="Keperluan..." required className="h-12 rounded-2xl text-xs border-muted-foreground/20 font-bold" />
              </div>
            </div>
            <DialogFooter className="p-6 bg-muted/30 gap-2">
              <Button type="button" variant="ghost" onClick={() => setIsDialogOpen(false)} className="flex-1 rounded-full text-xs font-bold h-11">Batal</Button>
              <Button type="submit" className="flex-1 bg-secondary text-white rounded-full font-bold h-11 text-xs shadow-lg">Simpan</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
            ...
            <div className="bg-white flex flex-col">
              <DialogHeader className="p-6 bg-primary text-white relative">
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    <DialogTitle className="text-lg font-black uppercase tracking-tight">Detail Transaksi</DialogTitle>
                    <div className="flex items-center gap-1 opacity-70">
                      <Hash className="h-3 w-3" />
                      <span className="text-[8px] font-mono font-bold tracking-widest">{viewingTransaction.id.slice(0, 12).toUpperCase()}</span>
                    </div>
                  </div>
              ...
              <div className="px-6 py-8 space-y-8">
                <div className="flex flex-col items-center text-center space-y-3">
                  ...
                  <div className="space-y-1">
                    <p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">Jumlah Transaksi</p>
                ...
                <div className="grid gap-6">
                  <div className="space-y-2">
                    <Label className="text-[9px] font-black uppercase text-muted-foreground tracking-widest">Keterangan</Label>
                    <div className="p-4 bg-muted/20 rounded-2xl border border-muted/50">
                      <p className="text-xs font-bold text-primary leading-relaxed uppercase">{viewingTransaction.description}</p>
                    </div>
                  </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center">
                        {viewingTransaction.isClosed ? (
                          <Badge className="bg-green-100 text-green-700 border-none text-[8px] font-black uppercase px-2 py-0.5">Terkunci</Badge>
                        ) : (
                          <Badge variant="outline" className="text-[8px] font-black uppercase border-primary/20 text-primary px-2 py-0.5">Terbuka</Badge>
                        )}
                      </div>
                    </div>
                  </div>
              ...
              <DialogFooter className="p-6 bg-muted/10">
                <Button onClick={() => setViewingTransaction(null)} className="w-full bg-primary text-white rounded-full font-black h-12 shadow-lg text-[10px] uppercase tracking-widest">
                  Tutup
                </Button>
              </DialogFooter>
            </div>
